...
const SEARCH_INDEX = [
  ...
  { label: "Calendar", path: "/routine/calendar", terms: ["calendar", "weekly planner", "week", "month", "planning"], app: true },
  ...
  return (
    <div className="relative flex min-h-[100svh] w-full flex-col justify-center bg-background p-6 md:p-10">
      ...
      <div className="relative mx-auto w-full max-w-5xl">
        <Illustration className="absolute inset-0 h-[50vh] w-full text-primary opacity-[0.05] dark:opacity-[0.04]" />
        ...
        <div className="relative z-[1] pt-52 text-center">
          ...
          >
            <div className="relative w-full">
              ...
              <Input
                ...
                className="border-primary/25 bg-background pl-8 focus-visible:ring-primary/40"
              />
              {matches.length > 0 && (
                <ul className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border border-primary/25 bg-popover py-1 text-left shadow-lg">
                  {matches.map((item) => (
                    <li key={item.path}>
                      <button
                        type="button"
                        onClick={() => navigate(item.path)}
                        className="block w-full px-3 py-2 text-left text-sm text-foreground hover:bg-primary/10"
                      >
                        {item.label}{" "}
            ...
            <Button
              ...
              className="border-primary/30 text-primary hover:bg-primary/10 hover:text-primary disabled:opacity-50"
            >
              Search
            </Button>
          </form>
          ...
          <div className="mt-10 flex flex-col gap-y-3 gap-x-6 sm:flex-row sm:items-center sm:justify-center">
            <Button
              variant="secondary"
              onClick={() => navigate(-1)}
              className="group bg-primary/10 text-primary hover:bg-primary/[0.16]"
            >
              <ArrowLeft
            ...
            <Button
              asChild
              className="-order-1 bg-primary text-primary-foreground hover:bg-primary/90 sm:order-none"
            >
