 ...
 * to. Used in the onboarding survey so users aren't forced to scroll past 195
 * options including ones with virtually no student traffic (Botswana, Brunei,
 * Bosnia, etc.). Profile settings still allow the full ISO list.
 */
export const TOP_DESTINATION_COUNTRY_CODES = [
  "US", "GB", "CA", "AU", "DE", "FR", "NL", "CH", "SG", "HK",
  "JP", "KR", "CN", "IN", "AE", "IE", "NZ", "IT", "ES", "SE",
  "DK", "NO", "FI", "BE", "AT", "ZA", "BR", "MX", "IL", "MY",
] as const;

export const TOP_COUNTRIES: CountryOption[] = TOP_DESTINATION_COUNTRY_CODES
...
export function searchCountries(query: string, limit = 50): CountryOption[] {
  ...
  return ALL_COUNTRIES.filter((c) => c.name.toLowerCase().includes(q)).slice(0, limit);
}
