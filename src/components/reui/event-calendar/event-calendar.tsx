...
interface EventCalendarProps<TData = unknown>
  ...
  /*
   * MutableRefObject, not RefObject.
   *
   * Upstream targets React 19, where RefObject<T>.current is writable. This app
   * is on React 18, whose RefObject marks `current` readonly, so the assignment
   * in the effect below does not compile. Same runtime object either way —
   * useRef still returns exactly this.
   */
  apiRef?: MutableRefObject<EventCalendarApi<TData> | null>
  children?: ReactNode
}
