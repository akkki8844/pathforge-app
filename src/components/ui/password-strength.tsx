 ...
 *
 * Two notes on how this differs from the reference implementation it came from.
 *
 * It imports from `framer-motion`, not `motion/react`. Those are the same
 * library under two package names; this app already ships framer-motion v12 and
 * adding `motion` as well would have put a second copy of the animation runtime
 * in the bundle to render four bars.
 *
 * And it is drawn in the app's own tokens rather than in `stone-*`, so it
 * inherits the theme instead of carrying its own greys — the reference palette
 * would have read as a foreign component on both the light and dark themes.
 * The three tones stay: strength is the one place in this app where colour is
 * the information rather than decoration, and there is no second channel
 * carrying it.
 *
 * This meter is advice, not a gate. What is actually enforced at sign-up lives
 * in the zod schema on the page.
 */
...
/** Openers of the passwords that actually appear in breach corpora. */
const COMMON =
...
const RUN = /(.)\1{3,}/;
/** A walk up the keyboard or the alphabet. */
const RUN_UP =
...
export function usePasswordStrength(
  ...
  const state = useMemo(() => {
    ...
    // A guessable password is pinned to the bottom of the scale however many
    // rules it technically satisfies: "Password123!" clears all four and is
    // still in every wordlist.
    const score =
      value.length === 0
        ? 0
        : guessable
  ...
  // The live region only speaks once typing pauses. Announcing on every
  // keystroke makes a screen reader unusable while entering a password.
...
const TONES = {
  none: { bar: "bg-muted-foreground/40", text: "text-muted-foreground" },
  danger: { bar: "bg-destructive", text: "text-destructive" },
  caution: { bar: "bg-amber-500", text: "text-amber-700 dark:text-amber-400" },
  safe: { bar: "bg-emerald-500", text: "text-emerald-700 dark:text-emerald-400" },
} as const;
    ...
    <div className={cn("w-full", className)}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div key={i} className="relative h-1.5 overflow-hidden rounded-[2px] bg-muted">
            <motion.span
              className={cn(
                "absolute inset-0 origin-left rounded-[2px] transition-colors duration-200",
                tone.bar,
              )}
              initial={false}
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        {/* All five labels stacked in one grid cell and crossfaded, so the row
            never changes height and nothing below it moves as you type. */}
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-amber-700 dark:text-amber-400"
          initial={false}
