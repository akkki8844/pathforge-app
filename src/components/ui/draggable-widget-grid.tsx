 ...
 *
 * Imported from `framer-motion` rather than `motion/react`: they are the same
 * library under two names, and this app already ships framer-motion on every
 * route. Adding the other package would put a second copy of it in the bundle.
 */
...
export interface DraggableWidgetGridProps {
    ...
    gap?: number
    /**
     * Row height in px. Cells are square by default (row height = column
     * width), which is right for a board of app icons and too tall for a strip
     * of read-at-a-glance tiles on a wide screen.
     */
    rowHeight?: number
...
const Widget = memo(function Widget({
    ...
    const downAt = useRef(null as { x: number; y: number } | null)
    ...
    return (
        <motion.div
            ...
            /*
             * A drag must not also follow the link under the pointer. A click
             * must.
             *
             * This used to cancel every anchor click while the grid was
             * `editable` — and the grid is editable by default, so every tile
             * on the dashboard was dead to the mouse. The distinction is
             * movement, not editability: a click leaves the pointer where it
             * went down, a drag does not. `swallow` covers the same case from
             * the other side, for a drag that ends over a different widget.
             */
            onClickCapture={(e) => {
                const from = downAt.current
                downAt.current = null
                const dragged =
                    !!from && Math.hypot(e.clientX - from.x, e.clientY - from.y) > PRESS_SLOP
                if (handlers.swallow() || dragged) {
                    e.preventDefault()
                    e.stopPropagation()
                }
            }}
...
export function DraggableWidgetGrid({
    ...
    const toSlot = useCallback(
        (box: Box): Slot => {
            ...
            const rowStep = Math.round(rowHeight ?? unit) + gap
