 ...
 *
 * The keyframes and the two classes this file relies on
 * (`scroll-reel-char`, `scroll-reel-exit`) live in `src/index.css`
 * under the "scroll reel" block, together with the reduced-motion
 * override that holds every character at rest.
 * ---------------------------------------------------------------- */
...
export function ScrollReelTestimonials({
  ...
  /* An empty list has no featured tile to centre on and no quote to
   * read, so the reel is simply not a thing that exists. Callers that
   * may have nothing to show can hand the array straight over. */
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-foreground/15 bg-transparent p-0 text-foreground transition-[opacity,transform] duration-200 ease-swift hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <svg
